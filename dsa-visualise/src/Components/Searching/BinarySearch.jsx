...
function ArrayCard({ value, index, isCurrent, isFound }) {
  ...
  const dynamicClasses = isFound
    ? "bg-gradient-to-br from-green-300 to-green-500 ring-2 ring-green-800 text-black animate-glow-found"
    : isCurrent
...
export default function BinarySearchComponent() {
  ...
  const handleBinarySearch = async () => {
    ...
    if (isNaN(targ) || array.length === 0) return;
    ...
    const createLogBox = (msg, color, emoji) => {
      const box = document.createElement("div");
      box.textContent = emoji ? `${emoji} ${msg}` : msg;
      box.className = `w-full p-4 mt-3 text-lg font-semibold rounded-xl border-l-8 shadow-xl bg-gradient-to-br from-[#f4f9f8] to-[#e9f1f2] ${color} animate-fade-in`;
    ...
    while (left <= right) {
      ...
      setCurrentIndex(mid);
      logDiv.appendChild(createLogBox(`${targ} is checking at index ${mid}`, "border-blue-400 text-blue-900", "🔍"));
      await new Promise((res) => setTimeout(res, 1000 - speed * 9.5));

      if (array[mid] === targ) {
        setFoundIndex(mid);
        logDiv.appendChild(createLogBox(`${targ} is found at index ${mid}`, "border-green-500 text-green-700", "✅"));
        setCurrentIndex(null);
        return;
      } else if (array[mid] < targ) {
        logDiv.appendChild(createLogBox(`${targ} > ${array[mid]} → Move Right`, "border-yellow-400 text-yellow-500", "➡️"));
        left = mid + 1;
      } else {
        logDiv.appendChild(createLogBox(`${targ} < ${array[mid]} → Move Left`, "border-yellow-400 text-yellow-500", "⬅️"));
        right = mid - 1;
      }
    }
    logDiv.appendChild(createLogBox(`${targ} is not Found`, "border-red-500 text-red-600", "❌"));
    setCurrentIndex(null);
  };
          ...
          {show && (
            <div className="mb-6">
              ...
              <button
                ...
                className="mt-4 px-5 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg shadow-lg"
              >
                Submit
              </button>
            </div>
          ...
          {array.length > 0 && (
            <>
              ...
              <div className="bg-gradient-to-br from-[#c9c7c4] to-[#eeeff2] min-h-[300px] border border-slate-200 p-8  shadow-xl">
                <h2 className="text-2xl font-semibold mb-6 text-center">Array Visualisation:</h2>
                ...
                <div id="log" className="mt-6 space-y-3"></div>
              </div>
            </>
          )}
        </div>
